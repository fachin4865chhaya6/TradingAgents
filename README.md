...
- **Observation (2026-05)**: Ran a quick 5-ticker spot check (AAPL, MSFT, NVDA, AMZN, GOOGL) — `deepseek-chat` was ~6x cheaper than `gpt-4o-mini` per run with broadly similar directional calls. Quality difference was most noticeable in the risk analyst section — GPT-4o-mini wrote more structured, nuanced caveats. Will keep using `deepseek-chat` for bulk screening and switch to `gpt-4o-mini` for final conviction checks.
- **Observation (2026-05)**: Sentiment analyst over-weighting confirmed on NVDA — a single earnings-week news spike skewed the whole report bullish. Added a note in `prompts/sentiment.py` to remind myself to patch the lookback window once I understand the prompt structure better.
